...
class PolicyGradient:
    ...
    def save(self):
        ...
        saver.save(self.session, "models/policy.ckpt")
...
def load_or_create_model():
    ...
    try:
        saver = tf.train.Saver()
        saver.restore(model.session, "models/policy.ckpt")
        return model
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            if os.path.isfile("data/reinforce_positive.txt"):
                with open('data/reinforce_positive.txt') as f:
                    for line in f:
                        line = line.split()
            ...
            if os.path.isfile("data/reinforce_negative.txt"):
                with open('data/reinforce_negative.txt') as f:
                    for line in f:
                        line = line.split()
                        data = article_parser.parse(line[0])
