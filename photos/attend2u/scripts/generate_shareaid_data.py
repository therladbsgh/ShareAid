...
CAPTION_TRAIN_JSON_FNAME = os.path.join(
    DATA_ROOT_PATH, 'json', 'brownuInstagramTrain_test.json'
)
CAPTION_TEST_JSON_FNAME = os.path.join(
    DATA_ROOT_PATH, 'json', 'brownuTopPhotos_test.json'
)

CAPTION_OUTPUT_PATH = os.path.join(DATA_ROOT_PATH, 'caption_dataset')

CAPTION_VOCAB_FNAME = os.path.join(
    CAPTION_OUTPUT_PATH, '%d_[not used].vocab' % (CAPTION_VOCAB_SIZE)
)
...
def save_data(train_data, test_data, output_path, rev_vocab):
  ...
  _save_data(
      train_data[0], train_data[1], os.path.join(output_path, "brownu-train.txt")
  )
  _save_data(
      test_data[0], test_data[1], os.path.join(output_path, "brownu-test.txt")
  )
