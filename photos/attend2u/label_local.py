...
# For when the GPUs are getting you down
...
flags.DEFINE_string('eval_dir', './checkpoints/labels',
...
def write_captions(image_labels, predictions):
  # photo to caption
  caption_json = {}
  print(predictions)
  label_file = open("rhcheng_labels.json", "w+")
  for i in range(len(predictions)):
    caption_json[str(i)] = predictions[i]
  json.dump(caption_json, label_file)
  label_file.close()
...
def _eval_once(saver, summary_writer, argmaxs, answer_ids, vocab, rev_vocab,
  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
	...
	print(desc)
        # desc is the ml predicted guess answer here
        answer_token_list.append(answer)
      write_captions(answer_token_list, desc_token_list)
    ...
    with tf.variable_scope(tf.get_variable_scope()) as scope:
      with tf.device('/cpu:0'):
          with tf.name_scope('%s_%d' % (TOWER_NAME, 0)) as scope:
            inputs = [
                tower_img_embedding[0],
                tower_context_length[0],
                tower_caption_length[0],
                tower_context_id[0],
                tower_caption_id[0],
                tower_answer_id[0],
                tower_context_mask[0],
                tower_caption_mask[0]
            ]
