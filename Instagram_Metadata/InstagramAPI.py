...
class InstagramAPI:
    ...
    def get_ranked_recepients(self):
        recipients = self.SendRequest('direct_v2/ranked_recipients?show_threads=true')
        return recipients

    def get_threads(self):
        direct = self.SendRequest('direct_v2/inbox/')
        return direct

    def get_recent_recepients(self):
        recipients = self.SendRequest('direct_share/recent_recipients/')
        return recipients

    def get_direct_thread(self, threadid):
        directthread = self.SendRequest('direct_v2/threads/' + threadid + '/?')
        return directthread
    ...
    def SendRequest(self, endpoint, post = None, login = False):
        ...
        if response.status_code == 200:
            ...
            if ((response.status_code == "429") or (response.status_code == 429)):
                print "Login successful --- now working on collecting the data.\n"
            else:
